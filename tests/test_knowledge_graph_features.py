...
# Stub knowledge-graph module to avoid heavy dependencies like ``networkx``.
kg_mod = types.ModuleType("analysis.knowledge_graph")
kg_mod.load_knowledge_graph = lambda path=None: None
kg_mod.risk_score = lambda g, c: 0.0
kg_mod.opportunity_score = lambda g, c: 0.0
...
def test_make_features_includes_knowledge_graph(monkeypatch):
    ...
    monkeypatch.setattr(
        feat.feature_gate, "select", lambda df, tier, regime, persist=False: (df, [])
    )
