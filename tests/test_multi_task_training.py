...
def test_joint_training_improves_metrics():
    ...
    f1_baseline = _f1_score(direction, (x1 > 0).astype(int))
    rmse_baseline = _rmse(vol, np.abs(x2))

    model = MultiHeadTransformer(2, num_symbols=1, d_model=16, nhead=2, num_layers=1, dropout=0.0)
    ...
    for _ in range(200):
        ...
        loss = (
            torch.nn.functional.binary_cross_entropy(out['direction'], y_dir)
            + torch.nn.functional.mse_loss(out['abs_return'], y_abs)
            + torch.nn.functional.mse_loss(out['volatility'], y_vol)
        )
        loss.backward()
        opt.step()
    with torch.no_grad():
        out = model(x_tensor, 0)
    f1_joint = _f1_score(direction, (out['direction'] > 0.5).int().numpy())
    rmse_joint = _rmse(vol, out['volatility'].numpy())
