...
install_pip_packages() {
  local install_meta_trader5="${MT5_INSTALL_META_TRADER5_PY:-0}"
  if [[ "${install_meta_trader5}" != "1" ]]; then
    echo "Skipping MetaTrader5 Python package installation (MT5_INSTALL_META_TRADER5_PY=${install_meta_trader5})."
    return
  fi
  ...
  if ! WINEPREFIX="${PY_PREFIX}" ${WINE} "${PYTHON_WIN_PATH}" -m pip show numpy >/dev/null 2>&1; then
    WINEPREFIX="${PY_PREFIX}" ${WINE} "${PYTHON_WIN_PATH}" -m pip install --only-binary :all: "numpy==1.26.4"
  else
    echo "numpy already present in Windows Python; skipping reinstall."
  fi

  if ! WINEPREFIX="${PY_PREFIX}" ${WINE} "${PYTHON_WIN_PATH}" -m pip show MetaTrader5 >/dev/null 2>&1; then
    WINEPREFIX="${PY_PREFIX}" ${WINE} "${PYTHON_WIN_PATH}" -m pip install --only-binary :all: "MetaTrader5<6"
  else
    echo "MetaTrader5 package already installed in Windows Python; skipping reinstall."
  fi
}
...
run_windows_python_check() {
  if [[ "${MT5_INSTALL_META_TRADER5_PY:-0}" != "1" ]]; then
    echo "Skipping MetaTrader5 Python verification (MT5_INSTALL_META_TRADER5_PY disabled)."
    return
  fi
...
main() {
  ...
  echo "pip show MetaTrader5: ${mt5_pkg_status}"
