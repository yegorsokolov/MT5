...
2. Ensure Python can discover the terminal. The Ubuntu installer now runs the
   discovery helper automatically and updates `.env`, but you can still export
   `MT5_TERMINAL_PATH` manually or place the terminal inside the repository's
   `mt5/` folder when building custom deployments.
3. `scripts/setup_ubuntu.sh` and the runtime bootstrap both invoke
   `python scripts/setup_terminal.py --install-heartbeat` automatically. Run the
   command manually when you need detailed diagnostics or want to supply
   explicit `--login`, `--password` and `--server` credentials for a headless
   login. The script reports the connected account, balance and broker when
   successful.
...
3. The runtime bootstrap starts the hourly artifact synchroniser by default
   (controlled via `START_ARTIFACT_SYNC`). If you disable it, schedule
   `python scripts/hourly_artifact_push.py` manually and ensure your Git
   credentials are cached so pushes succeed without prompts.
