...
def add_knowledge_graph_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ``graph_risk`` and ``graph_opportunity`` scores for each company. Missing
    entries default to ``0.0``.  The company identifier is taken from the
    ``Symbol`` column if present otherwise ``company``.  If neither column
    exists or the graph cannot be loaded, the input DataFrame is returned
    unchanged.
    """
    ...
    df = df.copy()
    df["graph_risk"] = companies.map(risk_map).fillna(0.0)
    df["graph_opportunity"] = companies.map(opp_map).fillna(0.0)
    return df
