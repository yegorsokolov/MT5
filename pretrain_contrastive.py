...
from models.contrastive_encoder import train_contrastive_encoder


def main() -> None:
    parser = argparse.ArgumentParser(description="Pre-train contrastive encoder")
    parser.add_argument("--config", type=Path, default=None, help="Path to config YAML")
    parser.add_argument(
        "--store-dir", type=Path, default=None, help="Optional model store directory"
    )
    parser.add_argument(
    ...
    data = torch.tensor(windows.reshape(windows.shape[0], -1), dtype=torch.float32)

    epochs = args.epochs or cfg.get("contrastive_epochs", 20)
    batch_size = args.batch_size or cfg.get("contrastive_batch_size", 32)
    train_contrastive_encoder(
        data, epochs=epochs, batch_size=batch_size, store_dir=args.store_dir
    )
