...
def _training_cmd(model: str) -> list[str]:
    if model == "nn":
        return ["python", "-m", "mt5.train_cli", "neural", "--resume-online"]
    if model == "rl":
        return ["python", "-m", "mt5.train_rl"]
    return ["python", "-m", "mt5.train", "--resume-online"]
...
def run_drift_detection() -> None:
    ...
    try:
        from mt5.monitor_drift import monitor
